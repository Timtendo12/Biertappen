...
<a href="/admin/packs/{{$pack->id}}" id="pack-{{$pack->id}}" class="rounded-md w-72 h-32 hover:cursor-pointer shadow shadow-2xl"
   style="background-image: url('{{$pack->image}}')">
    <div class="absolute text-white px-4 pt-5 pb-3 flex flex-col w-72 h-32 justify-between">
        <h1 class="font-bold text-xl">{{$pack->name}}</h1>
        <p class="italic text-xs overflow-ellipsis overflow-hidden">{{(count_chars($pack->description) > $maxChars ? substr($pack->description, 0, $maxChars) . "..." : $pack->description)}}</p>
        <div class="text-xs flex flex-row justify-between">
            <p>{{$pack->creator}}</p>
            <p>{{Carbon::parse($pack->created_at)->format('Y-m-d')}}</p>
        </div>
    </div>
    {{--Debug only--}}
    <p class="text-white text-xs p-1 font-mono">{{$pack->uuid}}</p>
</a>
